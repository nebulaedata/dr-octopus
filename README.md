...
开发环境、代码约定与 PR 流程见 [贡献指南](./CONTRIBUTING.md)。部署边界、凭据保护及漏洞私密报告方式见 [安全政策](./SECURITY.md)。
...
当前仅接入 Git 版本发版；构建产物使用 `pnpm release:build`，npm 与 GitHub Release 发布尚未自动化。完整命令、预发布和重试说明见 [CLI 版本发版](./apps/cli/README.md#版本发版)。

## 架构要点

- [系统架构概览](./docs/architecture/overview.md)
- [组件模型](./docs/architecture/component-model.md)
- [数据流](./docs/architecture/data-flow.md)
- [技术选型、风险与路线图](./docs/architecture/technology-risks-roadmap.md)
- [架构决策记录（ADRs）](./docs/adr/README.md)
...
依赖方向必须始终由宿主指向 `@octopus/agent/rpc`。RPC 基础层不得反向依赖宿主，也不应吸收具体产品策略。详细决策见 [ADR-0001：采用 Pi RPC 模式作为 Agent 与宿主间的桥梁](./docs/adr/0001-pi-rpc-host-architecture.md)。
...
完整的目录约定、分层边界、工厂装配、Pi surface 选择、生命周期和测试要求见 [Agent Inline Extension 开发指南](./packages/agent/src/extensions/README.md)。Workspace 是当前参考实现，具体业务边界见 [Workspace 架构](./docs/architecture/octopus-workspace.md)。
...
本项目原创代码采用 [MIT License](./LICENSE)。第三方代码、二进制工具和素材保留各自的许可证与权利，详见 [第三方声明](./THIRD_PARTY_NOTICES.md)。
