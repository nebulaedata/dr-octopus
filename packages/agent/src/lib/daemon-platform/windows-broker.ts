...
import { resolvePlatformGuard } from './guard-entry.js';
...
} catch { [Console]::Error.WriteLine('Daemon WMI launch failed'); exit 1 }
`;
...
function invokeBroker(request: object): Promise<{ code: number; pid: number }> {
  return new Promise((resolve, reject) => {
    const child = execFile(
      ...
      (error, stdout) => {
        if (error) {
          reject(new ProcessLifecycleError('PROCESS_DETACH_UNSUPPORTED', 'Local WMI launcher unavailable'));
          return;
        }
        try {
          ...
          reject(
            new ProcessLifecycleError('PROCESS_DETACH_UNSUPPORTED', 'Local WMI launcher refused creation')
          );
        }
      }
    );
 ...
 * The guard refuses different users and restrictive Jobs before executing the requested entry.
 */
export async function launchWindowsBroker(
  ...
  const directory = await mkdtemp(join(tmpdir(), 'octopus-daemon-launch-'));
  ...
  try {
    const env = {
      ...
      OCTOPUS_PROCESS_LAUNCH_NONCE: nonce,
      OCTOPUS_PROCESS_LAUNCH_USER: userInfo().username,
    };
    const command = [process.execPath, '--import', resolvePlatformGuard(), ...args].map(quote).join(' ');
    ...
    do {
      try {
        const value = JSON.parse(await readFile(receipt, 'utf8')) as {
          nonce?: unknown;
          pid?: unknown;
          error?: unknown;
        };
        if (value.nonce === nonce && value.pid === result.pid) {
          if (typeof value.error === 'string') {
            throw new ProcessLifecycleError('PROCESS_DETACH_UNSUPPORTED', value.error);
          }
          return result.pid;
        }
      } catch (error) {
