...
import { readMemoryMetadata } from '../lib/profile.js';
import type { MemoryProfile } from '../lib/profile.js';
import type { MemoryEndpoint } from '../definitions/lifecycle.js';

/**
 * Validate every endpoint before constructing a loopback URL.
 */
export async function discoverMemory(profile: MemoryProfile): Promise<MemoryEndpoint | null> {
  const endpoint = await readMemoryMetadata<MemoryEndpoint>(join(profile.directory, 'endpoint.json'));
  if (!endpoint) {
    return null;
  }
  if (
    ...
    throw new MemoryError('MEMORY_VERSION_CONFLICT', '记忆服务身份或协议不兼容，请重启服务');
  }
  return endpoint;
}
...
export async function memoryRequest<T>(
  ...
  const response = await fetch(`http://127.0.0.1:${endpoint.port}/memory/v1/${route}`, {
    ...
    try {
      while (true) {
        ...
        if (!(bytes instanceof Uint8Array) || (size += bytes.byteLength) > 4 * 1024 * 1024) {
          await reader.cancel();
          throw new MemoryError('MEMORY_RESPONSE_INVALID', '记忆服务响应超过限制');
        }
        parts.push(bytes);
      }
    } finally {
  ...
  if (!response.ok) {
    ...
    throw new MemoryError(failure.code ?? 'MEMORY_UNAVAILABLE', failure.message ?? '记忆服务不可用');
  }
  return value;
}
