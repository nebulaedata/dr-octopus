...
- 新界面先寻找同类页面与共享组件，局部需求通过 props、`className` 或 feature wrapper 实现。
...
- 业务状态必须同时有文字或图标含义，不能仅靠红、绿或状态点区分。状态到颜色、图标、文案的映射集中在各领域的 StatusBadge 组件（如 DocumentStatusBadge、ScheduleTaskStatusBadge），不在各处散落颜色判断。
...
- 菜单使用现有 DropdownMenu / Select 等组件；单选项展示当前值及选中状态。模型与思考等级继续复用合并选择器，避免创建第二套入口。
...
- 文案简短，使用对象名称和具体动作，如“查看日志”“停止全部”“重试”。沿用所在功能的语言与术语，模型名、命令和代码标识保留原名，并按需使用 `translate="no"` 防止自动翻译破坏标识符。
...
- 未确认完成的动作继续展示进行中或待确认，不能提前呈现成功。参考 [后台任务面板](./src/features/session/background-tasks/BackgroundTasksView.tsx) 的摘要、状态分组和按需日志。
