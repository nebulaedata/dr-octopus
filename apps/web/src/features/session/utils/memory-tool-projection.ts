...
export function projectMemoryTool(
  t: Translate,
  tool: ToolProjection
): MemoryToolDetailsProjection | undefined {
...
function memoryCoverageNotice(
  ...
  if (data.action === 'read') {
    return data.complete === false
      ? t(
          'session.memoryProjection.noticePartialBody',
          'Only part of the body was returned; later reads can complete it.'
        )
      : undefined;
  }
...
export function summarizeMemoryTool(tool: ToolProjection, t: Translate): string {
  ...
  if (result.action === 'read') {
    if (partial) {
      summary = t('session.memoryProjection.summaryReadablePartial', 'Showing {{count}} memories readable', {
        count: result.available,
      });
    } else {
      summary = t('session.memoryProjection.summaryReadable', '{{count}} memories readable', {
        count: result.available,
      });
    }
  } else if (partial) {
    summary = t('session.memoryProjection.summaryLeadsPartial', 'Showing {{count}} memory leads', {
      count: result.available,
    });
  } else {
    summary = t('session.memoryProjection.summaryLeads', '{{count}} memory leads', {
      count: result.available,
    });
  }
