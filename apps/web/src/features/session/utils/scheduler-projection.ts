...
export function projectSchedulerTool(t: Translate, tool: ToolProjection): SchedulerToolProjection {
  ...
  let detailTask: SchedulerTaskProjection | undefined;
  if (isRecord(details['task'])) {
    detailTask = projectTask(t, details['task']);
  } else if (looksLikeTask(details)) {
    detailTask = projectTask(t, details);
  } else {
  ...
  const tasks =
    tool.name === 'scheduler_list' ? items.map((item) => projectTask(t, item)).filter(hasTaskIdentity) : [];
...
function readNullableString<T extends string>(
  ...
  const candidate = value[source];
  if (candidate === null) {
    return { [target]: null } as Partial<Record<T, null>>;
  } else if (typeof candidate === 'string') {
    return { [target]: candidate.slice(0, 4_000) } as Partial<Record<T, string>>;
  } else {
    return {};
  }
}
