 ...
 */
import { coverageLocationsLabel } from './document-coverage-locations';
...
function findingStatusLabel(t: Translate, status: CoverageFinding['status']): string {
  switch (status) {
    ...
    case 'truncated':
      return t('session.coverage.findingStatus.truncated', 'Only partially included in text extraction; truncated');
    case 'unknown':
      return t('session.coverage.findingStatus.unknown', 'Coverage or result reliability could not be verified');
    default:
      return status;
  }
}
...
export function documentCoverageLabel(t: Translate, coverage?: DocumentCoverageV1): string | undefined {
  ...
  const limits: Record<DocumentCoverageV1['limits'][number], string> = {
    ...
    diagnostics: t('session.coverage.limit.diagnostics', 'coverage record limit reached; some details not listed'),
  };
  ...
  if (coverage.findingsOmitted) {
    sections.push(
      t('session.coverage.detailsOmitted', 'Some coverage details were omitted; the current list is not complete.')
    );
  }
...
function findingLabel(t: Translate, finding: CoverageFinding): string {
  ...
  if (finding.locationsOmitted) {
    lines.push(
      t('session.coverage.locationsOmitted', 'Some locations were omitted; the locations shown are not the complete list.')
    );
  }
  return lines.join('\n');
}
