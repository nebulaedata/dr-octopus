...
import { projectGoalToolDetails } from '@/features/session/utils/goal-projection';
...
export function GoalToolRenderer({ tool }: ToolRendererProps) {
  ...
  if (tool.name === 'goal_complete') {
    primaryLabel = t('session.goalTool.completionEvidence', 'Completion evidence');
  } else if (tool.name === 'goal_blocked') {
    primaryLabel = t('session.goalTool.blocker', 'Blocker');
  } else {
    primaryLabel = t('session.goalTool.waiting', 'Waiting');
  }
