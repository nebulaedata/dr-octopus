...
import type { MemoryEntryProjection } from '@/features/session/utils/memory-tool-projection';
    ...
    <Item variant="muted" role="listitem" className="items-start gap-3 p-4">
      ...
      <ItemContent className="min-w-0 gap-2.5">
              ...
              <div className="mt-3 flex max-h-48 flex-col gap-3 overflow-auto overscroll-contain">
                {entry.sources.length ? (
                  entry.sources.map((source, sourceIndex) => (
                    ...
                    >
                      {source ||
                        t('session.memoryTool.entry.sourceEmpty', 'This source has no text excerpt.')}
                    </blockquote>
                  ))
                ) : (
        ...
        {entry.truncated && (
          <p className="text-xs text-muted-foreground">
            {t(
              'session.memoryTool.entry.truncated',
              'Content shortened; see the raw result for full output.'
            )}
          </p>
        )}
      </ItemContent>
