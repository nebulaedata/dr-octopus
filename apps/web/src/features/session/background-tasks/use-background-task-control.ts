  ...
  return useMemoizedFn(async (action: BackgroundTaskAction, taskId?: string): Promise<void> => {
    ...
    await awaitRealtimeCommand(realtimeClient, requestId, () => {
      if (action === 'all') {
        ...
        if (!taskId) {
          throw new Error(t('session.backgroundTasks.control.taskMissing', 'The task was not found; reopen the task list.'));
        }
        send({
          ...target,
          type: 'agent.prompt',
    ...
    if (
      ...
      throw new Error(t('session.backgroundTasks.control.sessionChanged', 'The session state changed; reopen the task list and try again.'));
    }
    if (
      action === 'logs' &&
      (snapshot.error ||
      ...
      throw new Error(t('session.backgroundTasks.control.logsStale', 'The latest logs have not arrived yet; try again.'));
    }
    ...
    if (
      ...
      throw new Error(t('session.backgroundTasks.control.stopUnconfirmed', 'The stop is not confirmed yet; try again later.'));
    }
  });
}
