...
import { describeBackgroundTask } from './task-presentation';
import type { BackgroundTaskAction } from './use-background-task-control';
...
export function BackgroundTasksView({ sessionId, snapshot, onAction }: BackgroundTasksViewProps) {
  ...
  const summary =
    snapshot === null
      ? t('session.backgroundTasks.summary.pending', 'Status pending confirmation')
      : !snapshot.accepting
        ? t('session.backgroundTasks.summary.stopping', 'Stop pending confirmation')
        : active.length
          ? t('session.backgroundTasks.summary.active', '{{count}} running', { count: active.length })
          : failedCount
            ? t('session.backgroundTasks.summary.failed', '{{count}} tasks failed', { count: failedCount })
            : t('session.backgroundTasks.summary.recent', '{{count}} finished', { count: recent.length });
          ...
          >
            <span
              ...
              className={cn(
                ...
                needsAttention ? 'bg-destructive' : active.length ? 'bg-primary' : 'bg-muted-foreground/50'
              )}
            />
            {summary}
          </span>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:px-5">
            {(snapshot === null || !snapshot.accepting || error || snapshot.error) && (
              ...
              >
                ...
                <AlertTitle>
                  {error
                    ? t('session.backgroundTasks.alert.actionIncomplete', 'Action incomplete')
                    : pending === 'all:'
                      ? t('session.backgroundTasks.alert.stopping', 'Stopping tasks')
                      : t('session.backgroundTasks.alert.stopUnconfirmed', 'Stop not yet confirmed')}
                </AlertTitle>
                <AlertDescription>
                  {error ??
                    (snapshot === null
            ...
            {active.length > 0 || blocked ? (
              <>
                ...
                <Button variant="destructive" disabled={Boolean(pending)} onClick={() => void act('all')}>
                  ...
                  {pending === 'all:'
                    ? t('session.backgroundTasks.stoppingButton', 'Stopping…')
                    : blocked
                      ? t('session.backgroundTasks.retryStop', 'Retry stop')
                      : t('session.backgroundTasks.stopAll', 'Stop all')}
                </Button>
              </>
            ) : (
