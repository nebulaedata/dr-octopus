...
import {
  ...
  startAutoRetry,
} from '@/stores/session/reducers/retry-reducer';
import { mergeToolResultProjection } from '@/stores/session/utils/tool-result-projection';
...
import type { SessionProjectionState, ToolProjection } from '@/stores/session/type';
...
export function reduceEvent(
  ...
  if (envelope.type === 'agent.state' && isAgentStatePayload(envelope.payload)) {
    ...
    let stateChanges: Partial<SessionProjectionState>;
    ...
    } else if (envelope.payload.permission === undefined) {
      stateChanges = {};
    } else {
      stateChanges = { permission: envelope.payload.permission };
    }
  ...
  if (envelope.type === 'subagents.state') {
    if (envelope.payload === null) {
      return { ...next, subagentFleet: undefined };
    } else if (isSubagentFleetSnapshot(envelope.payload)) {
      return { ...next, subagentFleet: envelope.payload };
    } else {
      return next;
    }
  }
...
function endCompaction(
  ...
  let status: 'success' | 'error' | 'aborted';
