...
} from '../dist/modules/sessions/sessions.controller.js';
...
test('session codes render localized messages through the public error projection', () => {
  ...
  const busy = new ApplicationError('SESSION_BUSY', '当前任务或交互尚未结束，重启将中断任务。', {
    statusCode: 409,
  });
  assert.equal(toPublicError(busy, 'zh-CN').message, '当前任务或交互尚未结束，重启将中断任务。');
  assert.equal(
    toPublicError(busy, 'en').message,
    'A task or interaction is still running; restarting will interrupt it.'
  );
