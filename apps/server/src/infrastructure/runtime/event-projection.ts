...
export class RuntimeEventProjection {
  ...
  #publish(event: HostAgentEvent): void {
    ...
    let type: HostEventEnvelope['type'];
    ...
    let payload: unknown;
    if (subagentProjection.kind === 'snapshot') {
      payload = subagentProjection.snapshot ?? null;
    } else {
      if (type === 'extension.ui') {
        payload = this.#planQuestionnaires.project({
          ...event,
    ...
    const envelope: HostEventEnvelope = {
      ...
      type,
      payload,
      ...projectEventMemory(memory, event),
    };
    if (subagentProjection.kind === 'snapshot') {
...
/**
 * Preserves memory omission, explicit recovery clearing, and populated snapshots.
 */
function projectEventMemory(
  memory: HostEventEnvelope['memory'],
  event: { type: string; payload: unknown }
): Pick<HostEventEnvelope, 'memory'> {
  if (memory) {
    return { memory };
  }
  if (event.type === 'runtime-state' && isRecoveringState(event.payload)) {
