...
function manifestAttributes(item: ResolvedAgentAttachments['manifests'][number]): string {
  ...
  let extractionScope = 'metadata-only';
  if (schema !== undefined) {
    extractionScope = 'text-with-locators';
  } else if (item.detectedMediaType.startsWith('image/') && item.contentAvailableToModel) {
    extractionScope = 'image-input';
  }
...
export function attachmentDeliveryPrompt(manifests: ResolvedAgentAttachments['manifests']): string {
  ...
  if (manifests.some((item) => item.detectedMediaType.startsWith('image/'))) {
    prompt +=
      '\n<host_image_reference_workflow>For image generation or editing with image_generate, pass the attachment original_path as a referenceImages entry. A manifest-only image has not been visually inspected by the conversation model. Do not claim to have viewed it. Preserve the source and use the image tool default output directory unless the user specifies another destination.</host_image_reference_workflow>';
  }
  return prompt;
}
