...
import type { SessionRuntimeCoordinator, SessionRuntimeBinding } from '../../lib/runtime/index.js';
import type {
...
export class EffectiveSkillsService {
  ...
  public constructor(
    ...
    options: EffectiveSkillsServiceOptions
  ) {
  ...
  async #toEffectiveSkill(
    ...
    const managedScope = (await isPathOwnedBy(sourceInfo.path, workspaceRoot))
      ? 'workspace'
      : (await isPathOwnedBy(sourceInfo.path, globalRoot))
        ? 'global'
        : undefined;
    return {
      name,
