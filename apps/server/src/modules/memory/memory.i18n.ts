/**
 * @author Codex
 * @description Bilingual public message catalog for memory-domain error codes.
 * Memory-owned codes declare bilingual generic variants; codes shared with other domains
 * (CANCELLED, INVALID_INPUT, NOT_FOUND, REVISION_CONFLICT) contribute site variants only.
 */
...
export const memoryErrorMessages: ErrorMessageCatalog = {
  ...
  READ_ONLY: [
    ...
    { match: '此 Agent 只能读取记忆。', en: 'This Agent can only read memories.', 'zh-CN': '此 Agent 只能读取记忆。' },
    {
      match: '当前模式未启用记忆召回。',
  ...
  STORE_UNAVAILABLE: [
    { en: 'Memory storage is temporarily unavailable; try again later.', 'zh-CN': '记忆存储暂不可用，请稍后重试。' },
