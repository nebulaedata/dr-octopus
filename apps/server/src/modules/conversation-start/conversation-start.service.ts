...
import type { ChannelService } from '../channel/index.js';
...
export class ConversationStartService {
  ...
  async #run(row: Row): Promise<void> {
    ...
    try {
      ...
      repository.accept(row.submissionId, () => {
...
export class ConversationModelsService {
  ...
  async resolve(selection: ModelSelection) {
    ...
    let selected: (typeof models)[number] | undefined;
    if (provider && id) {
      selected = models.find((model) => model.provider === provider && model.id === id);
    } else {
      if (!defaults.configured && models.length === 1) {
        selected = models[0];
      } else {
        selected = undefined;
      }
