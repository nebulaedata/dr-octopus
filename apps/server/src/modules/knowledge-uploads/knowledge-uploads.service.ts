...
import type { KnowledgeUploadsRepository } from './knowledge-uploads.repository.js';
...
export class KnowledgeUploadStore extends DataStore {
   ...
   */
  constructor(
    private readonly repository: KnowledgeUploadsRepository,
    root: string,
  ...
  override async create(file: Upload): Promise<Upload> {
    ...
    const requestKey = createHash('sha256')
      ...
      .digest('hex');
    const previous = this.repository.findByRequestKey(requestKey);
    if (previous) {
    ...
    const winner = this.repository.findByRequestKey(requestKey)!;
  ...
  override async write(source: Readable, id: string, offset: number): Promise<number> {
    ...
    try {
      ...
      this.repository.updateOffset(id, next);
      return next;
    } catch (error) {
      await truncate();
      throw error;
  ...
  finalize(id: string, workspaceId?: string): Promise<{ sha256: string }> {
    ...
    const pending = (async () => {
      ...
      this.repository.markPublished(id, blob.sha256);
  ...
  override async deleteExpired(): Promise<number> {
    await this.ready;
    const rows = this.repository.listExpired();
