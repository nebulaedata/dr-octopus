  ...
  const transport: Transport =
    typeof entry.command === 'string'
      ...
      : typeof entry.socket === 'string'
        ? new UnixSocketProbeTransport(resolveRequiredPath(entry.socket, cwd))
        : (() => {
...
async function probeHttp(
  ...
  const transports =
    entry.httpTransport === 'sse'
      ...
      : entry.httpTransport === 'streamable-http'
        ? [() => new StreamableHTTPClientTransport(url, options)]
        : [() => new StreamableHTTPClientTransport(url, options), () => new SSEClientTransport(url, options)];
  let needsAuth = false;
...
function resolveRequiredPath(value: string, cwd: string): string {
  ...
  const homeExpanded =
    expanded === '~'
      ? homedir()
      : expanded.startsWith('~/')
        ? resolve(homedir(), expanded.slice(2))
        : expanded;
